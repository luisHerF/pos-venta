    ...
    try {
      if (mode === 'signin') {
        ...
        if (data.user) {
          ...
          let role: string = 'sin_asignar'
          if (code.includes(':')) {
            const [bizId, r] = code.split(':')
            if (['admin', 'vendedor', 'inventarista'].includes(r)) {
              business_id = bizId
              role = r
            }
          }
          ...
          {mode === 'signup' && (
            <>
              ...
              <div className="mb-3">
                ...
                <input className="input" value={inviteCode} onChange={(e) => setInviteCode(e.target.value)} placeholder="Ej. abcd1234-...:vendedor (te lo da tu administrador)" />
                <p className="text-xs text-gray-400 mt-1">¿Eres el super administrador? Regístrate sin código y pide que te asciendan.</p>
              </div>
            </>
          )}
